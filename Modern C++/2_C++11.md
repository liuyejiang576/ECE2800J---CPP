...
### 1.10 Move Semantics in Practice
...
int main() {
    ...
    // Move construction into vector
    widgets.emplace_back("Widget1", 42);
    widgets.emplace_back("Widget2", 84);
    
    // Move assignment
    Widget temp("Temp", 99);
    widgets[0] = std::move(temp);
...
public:
    ...
    ~Resource() {
        std::cout << "Resource released: " << name << "\n";
    }
    
    Resource(const Resource&) = delete;              // No copy constructor
    Resource& operator=(const Resource&) = delete;   // No copy assignment
    
    // Move constructor
    Resource(Resource&& other) noexcept : name(std::move(other.name)) {
        std::cout << "Resource moved: " << name << "\n";
    }
    ...
    Resource& operator=(Resource&& other) noexcept {
        ...
        std::cout << "Resource move assigned: " << name << "\n";
        return *this;
    }
};
...
int main() {
    ...
    if (ptr1) {
        std::cout << "ptr1 still owns resource\n";
    } else {
...
### 1.11 Perfect Forwarding in Practice 
...
Move semantics represent one of the most important advances in C++11, enabling modern C++ code to be both high-performance and easy to write correctly. They are most beneficial when:
...
In traditional C++, `NULL` is typically defined as `0` (integer), which can cause serious issues when function overloading is involved:
...
int main() {
    process(0);        // Calls process(int) - probably intended
    process(NULL);     // Calls process(int) - likely unintended!
    // NULL is just 0, so it matches int better than int*
}
```

**The Core Problem**: `NULL` is typically defined as `0` or `(void*)0`, making it an integer rather than a true null pointer. This breaks function overloading and can lead to unexpected behavior.

### 6.2 nullptr - The Type-Safe Null Pointer

C++11 introduced `nullptr`, a literal of type `std::nullptr_t` that is implicitly convertible to any pointer type but not to integer types:
...
int main() {
    ...
    process(NULL);     // Calls process(int) - ambiguous!
    process(nullptr);  // Calls process(int*) - correct!
    
    // Clear and unambiguous
    int* p = nullptr;
    if (p == nullptr) {  // Clear intent
        std::cout << "p is null\n";
    }
...
int main() {
    std::cout << "nullptr type: " << typeid(nullptr).name() << "\n";
    std::cout << "std::nullptr_t: " << typeid(std::nullptr_t).name() << "\n";
    std::cout << "Same type: " << std::is_same_v<decltype(nullptr), std::nullptr_t> << "\n";
    
    // nullptr_t is implicitly convertible to any pointer type
    int* p1 = nullptr;
    double* p2 = nullptr;
...
#### Practical Examples with nullptr
...
public:
    void addResource(std::unique_ptr<Resource> res) {
        if (res != nullptr) {  // Clear null check
            resources.push_back(std::move(res));
        } else {
            std::cout << "Cannot add null resource\n";
        }
    }
    ...
    Resource* getResource(const std::string& name) {
        ...
        return nullptr;  // Clear return of null pointer
    }
    
    void useResource(const std::string& name) {
...
int main() {
    ...
    // Add resources
    manager.addResource(std::make_unique<Resource>("Database"));
    manager.addResource(std::make_unique<Resource>("Cache"));
    manager.addResource(nullptr);  // Won't be added
    
    // Use resources
    manager.useResource("Database");
    manager.useResource("NonExistent");  // Won't be found
    
    // Clear pointer
    Resource* db = manager.getResource("Database");
    db = nullptr;  // Clear intent
}
```

### 6.3 Strongly-Typed Enums (enum class)

Traditional enums have several problems that `enum class` solves:

#### Problem 1: Enumerator Name Pollution

```c++
enum Color { RED, GREEN, BLUE };
enum Size { SMALL, MEDIUM, LARGE };

// Problem: Enumerators leak into enclosing scope
int x = RED;      // OK, but might conflict with other RED definitions
int y = SMALL;    // OK, but confusing - which enum does SMALL belong to?

// Worse: name conflicts
enum Status { SUCCESS, FAILURE };
enum Result { SUCCESS, ERROR };  // Compilation error: SUCCESS already defined
```
...
void processColor(int color) {
    std::cout << "Processing color as integer: " << color << "\n";
}

void processColor(Color color) {
    std::cout << "Processing color enum\n";
}

int main() {
    Color c = RED;
    processColor(c);  // Calls processColor(int) - probably not intended!
...
// This compiles but doesn't make sense
if (c == s) {
    std::cout << "Color equals size\n";  // Nonsensical comparison
}
```

### 6.4 enum class - The Solution

C++11 introduced `enum class` (scoped enums) that solve all these problems:

#### 1. Scoped Enumerators
...
int main() {
    ...
    // No name pollution
    // int x = RED;  // Error: RED not in scope
    // int y = SMALL;  // Error: SMALL not in scope
    
    // No conflicts between different enums
    enum class Status { SUCCESS, FAILURE };
    enum class Result { SUCCESS, ERROR };  // OK: different scopes
}
```

#### 2. No Implicit Integer Conversion
...
int main() {
    Color c = Color::RED;
    processColor(c);  // Calls processColor(Color) - unambiguous!
    
    // Explicit conversion required
    int value = static_cast<int>(Color::RED);
    processColor(value);  // Now calls processColor(int)
}
```
...
// if (c == s) { }  // Error: cannot compare Color and Size

// Must be explicit about conversion
if (static_cast<int>(c) == static_cast<int>(s)) {
    std::cout << "Same underlying value\n";
}
```

### 6.5 Advanced enum class Features

#### 1. Specifying Underlying Type
...
int main() {
    ...
    std::cout << "ErrorCode size: " << sizeof(ErrorCode) << "\n";
    std::cout << "Priority size: " << sizeof(Priority) << "\n";
}
```
...
```c++
// Forward declaration with specified underlying type
enum class Color : int;

// Later definition
enum class Color : int { RED, GREEN, BLUE };

// Usage in header files
class Widget {
    Color color;
public:
...
bool hasPermission(Permission p, Permission check) {
...
int main() {
    ...
    std::cout << "Has READ: " << hasPermission(userPerm, Permission::READ) << "\n";
    std::cout << "Has EXECUTE: " << hasPermission(userPerm, Permission::EXECUTE) << "\n";
}
```

### 6.6 Practical Examples

#### Example 1: State Machine with enum class
...
public:
    ...
    bool connect(const std::string& serverAddr) {
        if (state != ConnectionState::DISCONNECTED) {
            std::cout << "Cannot connect: already connected or connecting\n";
            return false;
        }
        
        state = ConnectionState::CONNECTING;
        std::cout << "Connecting to " << serverAddr << "...\n";
    ...
    bool disconnect() {
        if (state != ConnectionState::CONNECTED) {
            std::cout << "Cannot disconnect: not connected\n";
            return false;
        }
...
int main() {
    ...
    std::cout << "Initial state: " << static_cast<int>(conn.getState()) << "\n";
    
    conn.connect("example.com");
    std::cout << "After connect: " << static_cast<int>(conn.getState()) << "\n";
    
    conn.disconnect();
    std::cout << "After disconnect: " << static_cast<int>(conn.getState()) << "\n";
}
```

#### Example 2: Configuration with enum class
...
public:
    Logger(LogLevel l = LogLevel::INFO, OutputType o = OutputType::CONSOLE)
        : level(l), output(o) {}
    
    void setLevel(LogLevel l) { level = l; }
    void setOutput(OutputType o) { output = o; }
        ...
        if (msgLevel >= level) {
            ...
            switch (output) {
                ...
                case OutputType::FILE:
                    // Write to file
                    std::cout << "[FILE] " << prefix << " " << message << "\n";
                    break;
                case OutputType::SYSLOG:
                    // Write to syslog
                    std::cout << "[SYSLOG] " << prefix << " " << message << "\n";
                    break;
            }
        }
    }
...
int main() {
    Logger logger(LogLevel::DEBUG, OutputType::CONSOLE);
    ...
    logger.setOutput(OutputType::FILE);
    
    logger.debug("This won't be shown");
    logger.error("This will be shown in file format");
}
```

### 6.7 Best Practices

#### 1. Always Use enum class Instead of enum
...
#### 3. Provide Helper Functions for enum class

```c++
enum class Status { SUCCESS, FAILURE, TIMEOUT };
...
#### 4. Use Explicit Underlying Types When Needed

```c++
// When you need specific size or values
...
`nullptr` and `enum class` are essential features that improve type safety and code clarity:

**nullptr Benefits**:
- **Type-safe null pointer**: Cannot be confused with integer 0
- **Clear intent**: Makes null pointer checks explicit
- **Better overloading**: Works correctly with function overloading
- **Universal compatibility**: Works with all pointer types

**enum class Benefits**:
- **Scoped enumerators**: No name pollution in enclosing scope
- **Type safety**: Cannot be implicitly converted to integers
- **No conflicts**: Different enum classes can have same enumerator names
- **Explicit conversions**: Required for integer operations

**Key Principles**:
1. Always prefer `nullptr` over `NULL` or `0` for null pointers
2. Always prefer `enum class` over `enum` for new code
3. Use explicit underlying types when you need specific memory layout
4. Provide helper functions for common enum operations
5. Use structured bindings and range-based for loops with enum values when appropriate

These features make C++ code safer, more readable, and less prone to subtle bugs that were common in pre-C++11 code.

---

## Chapter 7: Uniform Initialization and std::initializer_list
...
### 7.8 Common Pitfalls and Solutions
...
Uniform initialization with braces `{}` is a fundamental feature that:

**Solves Key Problems**:
- **Consistency**: One syntax for all types
- **Safety**: Prevents narrowing conversions
- **Clarity**: Eliminates most vexing parse ambiguity
- **Flexibility**: Works with all initialization contexts

**Key Benefits**:
1. **Type Safety**: Prevents accidental data loss through narrowing
2. **Consistency**: Same syntax for built-in types, classes, and containers
3. **Clarity**: Eliminates parsing ambiguities
4. **Modern**: Aligns with other C++11+ features

**Best Practices**:
1. **Prefer braces** `{}` over parentheses `()` for initialization
2. **Use std::initializer_list** for custom container constructors
3. **Be explicit** about constructor overloads to avoid ambiguity
4. **Understand** the difference between direct and copy list initialization
5. **Test** your code with various initialization forms to ensure expected behavior
...
### 8.1 The Problem with Fixed-Parameter Templates
...
**The Core Problem**: Creating truly generic functions that accept any number of arguments required writing multiple overloads or using macros, which were error-prone and didn't provide type safety.

### 8.2 Parameter Packs - The Foundation

C++11 introduced variadic templates with parameter packs, allowing templates to accept zero or more template parameters:
...
### 8.3 Recursive Template Expansion
...
void print(T first, Args... rest) {
    ...
    print(rest...);  // This expands to: print(arg2, arg3, ..., argN)
}

// When called with: print(1, 2, 3)
// 1. T = int, first = 1, Args... = int, int, rest... = 2, 3
// 2. Prints "1, "
// 3. Calls print(2, 3)
// 4. T = int, first = 2, Args... = int, rest... = 3
// 5. Prints "2, "
// 6. Calls print(3)
// 7. T = int, first = 3, Args... = (empty), rest... = (empty)
// 8. Prints "3"
// 9. Calls print() (base case)
// 10. Prints "(no more arguments)"
```
...
### 8.5 Practical Variadic Template Examples
...
#### Example 3: Tuple-like Container
...
### 8.6 Advanced Variadic Template Patterns

#### 1. SFINAE with Variadic Templates
...
### 8.7 Best Practices with Variadic Templates

#### 1. Prefer Fold Expressions When Possible
...
#### 4. Use SFINAE for Type Constraints
...
#### Pitfall 1: Template Argument Deduction
...
Variadic templates are a powerful feature that enables truly generic programming in C++:

**Key Benefits**:
1. **Flexibility**: Functions and classes that work with any number of arguments
2. **Type Safety**: Compile-time type checking for all arguments
3. **Performance**: Zero runtime overhead through template instantiation
4. **Expressiveness**: Clean, readable code for complex generic operations

**Core Concepts**:
1. **Parameter Packs**: `typename... Args` syntax for accepting multiple types
2. **Pack Expansion**: `args...` syntax for expanding parameter packs
3. **Recursive Processing**: Base case + recursive case pattern
4. **Fold Expressions**: C++17 feature for cleaner parameter pack processing

**Best Practices**:
1. **Use fold expressions** when possible for cleaner code
2. **Apply perfect forwarding** to preserve value categories
3. **Handle edge cases** like empty parameter packs
4. **Use SFINAE** for compile-time type constraints
5. **Consider compilation performance** with deep recursion
